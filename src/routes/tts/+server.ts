...
export const POST: RequestHandler = async ({ request, locals }) => {
    ...
    const result = v.safeParse(TTSSchema, body);
    if (!result.success) {
        error(400, 'Invalid request body');
    }

    const { voice_id, tts_msg } = result.output;
    ...
    if (audio_record) {
        ...
        audio_blob = await locals.tts_service.textToSpeech.convert(
    ...
    // return voice clip
    // return audio_blob;
    return new Response(audio_blob, {
